...
| `shkolakoda.com`, `www.shkolakoda.com` | Live; School of Code rebuild pending | nginx proxies HTTPS traffic to `127.0.0.1:8000`; HTTP unknown/default fallback redirects to `https://shkolakoda.com/` | `/etc/nginx/sites-available/shkolakoda.com` and `/etc/nginx/sites-available/default-shkolakoda.conf` -> enabled; repo artifacts: `deploy/nginx/shkolakoda.com`, `deploy/nginx/default-shkolakoda.conf` | `webgarden-shkolakoda.service` | `/var/www/webgarden/sites/shkolakoda` | None | No application env file identified; service supplies venv `PATH` | `/var/www/webgarden`; `git@github.com:evgeny-vlasov/my-web-garden.git` | Source plus nginx/systemd/default fallback artifacts through Web Garden backups | Intended School of Code app on port 8000. Public School of Code content has not yet been rebuilt, so this path may temporarily still contain legacy Happy Science content until the next approved phase. Keep IPv4 and IPv6 listen lines together. |
| `science.shkolakoda.com` | Prepared for deployment | nginx will proxy HTTPS traffic to `127.0.0.1:8005` after repo artifacts are installed | planned `/etc/nginx/sites-available/science.shkolakoda.com`; repo artifact: `deploy/nginx/science.shkolakoda.com` | planned `webgarden-science.service`; repo artifact: `deploy/systemd/webgarden-science.service` | `/var/www/webgarden/sites/science` | None | No application env file identified; service supplies venv `PATH` | `/var/www/webgarden`; `git@github.com:evgeny-vlasov/my-web-garden.git` | Source plus nginx/systemd artifacts through Web Garden backups | Preserved Happy Science app copied from `sites/shkolakoda`. Future focus is hands-on workshops and science labs; camps remain secondary or occasional. Uses the existing `shkolakoda.com` certificate, which already covers `science.shkolakoda.com`. |
