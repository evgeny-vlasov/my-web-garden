...
| `laic.ca`, `www.laic.ca` | Deployed | nginx proxies to local app on `127.0.0.1:8004` | `/etc/nginx/sites-available/laic.ca` → enabled | `webgarden-laic.service` active | `/var/www/webgarden/sites/laic` | PostgreSQL contact storage active: database `laic_db`, role `laic_user` | `/etc/webgarden/laic.env` | `/var/www/webgarden`; `git@github.com:evgeny-vlasov/my-web-garden.git` | Back up source, protected env, nginx, systemd, and PostgreSQL contact submissions through Web Garden backups | Public nonprofit-style site for Local AI Collective. Older demo/dashboard material is treated as archive/example content only. |
