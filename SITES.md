...
|---|---|---|---|---|---|---|---|---|---|---|
| `psyling.com`, `www.psyling.com` | Healthy, production-sensitive | Cloudflare is involved for at least the `www` route; apex reaches this VPS | `/etc/nginx/sites-available/psyling.com` → enabled | `webgarden-psyling.service` | `/var/www/webgarden/sites/psyling` (`/var/www/webgarden/sites/therapist` is a compatibility symlink) | PostgreSQL `therapist_db` | `/etc/webgarden/psyling.env` | `/var/www/webgarden`; `git@github.com:evgeny-vlasov/my-web-garden.git` | Nightly encrypted PostgreSQL dump; repository, migrations, protected environment, and upload backup; regular restore test | Contains private CRM, client, activity, and chat data. Do not expose database rows or raw logs. nginx access logging is disabled. |
