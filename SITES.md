...
|---|---|---|---|---|---|---|---|---|---|---|
| `psyling.com`, `www.psyling.com` | Healthy, production-sensitive | Cloudflare is involved for at least the `www` route; apex reaches this VPS | `/etc/nginx/sites-available/psyling.com` → enabled | `webgarden-psyling.service` | `/var/www/webgarden/sites/psyling` (compatibility symlink: `/var/www/webgarden/sites/therapist`) | PostgreSQL `therapist_db` | `/etc/webgarden/psyling.env` | `/var/www/webgarden`; `git@github.com:evgeny-vlasov/my-web-garden.git` | Nightly encrypted PostgreSQL dump; repository, migrations, protected environment, and upload backup; regular restore test | Contains private CRM, client, activity, and chat data. Do not expose database rows or raw logs. nginx access logging is disabled. |
...
| `laic.ca`, `www.laic.ca` | Source restored in Web Garden; deployment pending DNS cutover | Planned to proxy from this VPS once nginx/service are enabled; do not repoint DNS until verification is complete | Planned: `/etc/nginx/sites-available/laic.ca` → enabled | Planned: `webgarden-laic.service` | `/var/www/webgarden/sites/laic` | PostgreSQL optional; app currently supports PostgreSQL or local instance-file fallback for contact intake | Planned: `/etc/webgarden/laic.env` | `/var/www/webgarden`; `git@github.com:evgeny-vlasov/my-web-garden.git` | Back up source through Web Garden immediately; once deployed, include protected env, nginx, systemd, and contact submission storage in regular backups | Public nonprofit-style site for Local AI Collective. Older demo/dashboard material is treated as archive/example content only. Do not repoint DNS until an isolated restoration is reviewed and tested. |
